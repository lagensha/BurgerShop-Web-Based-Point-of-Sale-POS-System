...
function viewCart() {
  ...
  case 'Bacon_cheddar':
    ...
    break;
      ...
      case 'Hawaiian_burger':
    alert("Hawaiian pineapple burger added to cart");
    break;

    case 'mushroom_burger':
    alert("mushroom burger added to cart");
    break;

    case 'Veggie_cheddar':
    alert("Veggie_cheddar burger added to cart");
    break;
...
function removeItem(element,name) {
  ...
  subproducts = JSON.parse(localStorage.getItem("products"));
   const index = products.indexOf(name);
  if (index > -1) {
    subproducts.splice(index, 1);
  }

  localStorage.setItem("products", JSON.stringify(subproducts));
